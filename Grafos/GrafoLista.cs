...
namespace Grafos
{
    public class GrafoLista : Grafo
    {
        ...
        public override bool RemoverVertice(int indice)
        {
            ...
            {
                if (i != indice) _adj[i].RemoveAll(a => a.Destino == indice);
            }
        ...
        public override bool RemoverAresta(int origem, int destino)
        {
            ...
            if (!Direcionado) removed |= _adj[destino].RemoveAll(a => a.Destino == origem) > 0;
            return removed;
        }
        ...
        public override void ImprimeGrafo()
        {
            ...
            for (int i = 0; i < NumeroVertices; i++)
            {
                ...
                var arestas = _adj[i].Select(a => Ponderado
                        ? $"{a.Destino}(w={a.Peso.ToString("0.##", CultureInfo.InvariantCulture)})"
                        : $"{a.Destino}");
